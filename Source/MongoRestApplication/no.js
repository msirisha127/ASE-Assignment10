...
var app = express();
var result={'body': []};
var url = 'mongodb://sirisha127:Sirisha@123@ds139480.mlab.com:39480/demoase';
app.use(cors());
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('collection1').updateOne({"_id":ObjectId(id)},{$set:item}, function(err, result) {
            if(err)            {
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('collection1').deleteOne({"_id": ObjectId(id)}, function(err, result) {
